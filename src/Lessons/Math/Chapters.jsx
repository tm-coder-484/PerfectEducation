  ...
  return (
    <>
      ...
      <MathRenderer content={String.raw`
        ...
        To solve algebraic equations, you must isolate your variable by adding, subtracting, multiplying or dividing numbers on both sides. You can think of this as keeping a see saw balanced. For example to solve $x+4=10$, you first subtract 4 from both sides, as in order to get rif of the +4 we must take it away, so we get $x=10-4$ or $x=6$. For subtraction, we add values to both sides to cancel out the subtraction. For example to solve $x-1=2$, add 1 to both sides to get $x=2+1$ or $x=3$. For multipication, you divide on both sides to cancel out the multiplication, for example to solve $2x=4$, you divide by two on both sides to get $x=\frac{4}{2}$ or $x=2$. For divison, multiply on both sides. For example to solve $\frac{x}{4}=2$, you multiply by 4 on both sides to get $x=4\cdot 2$ or $x=8$.
        ## Verifying solutions
  ...
  return (
    <>
      ...
      <MathRenderer content={String.raw`
        ...
        Now you may be wondering how these weird looking functions could possibly help us to find the sides of a triangle. Which is why i shall show you how. First, lets look at some simpler equations. To solve $x/4 =2$, you would multiply both sides by 4 in order to get $x=8$, Now lets apply this logic to our triangles.

      `} />
      <img src={Tri1} className='trig-image' />
      <MathRenderer content={String.raw`
        Our given sides are the opposite(x, because its opposite the given angle 32) and the hypotenuse(4), therefore we use sin because $\sin(\theta)=\frac{opposite}{hypotenuse}$. Now sub in the values for $\theta$, the opposite side and the hypotenuse to get $\sin32=\frac
